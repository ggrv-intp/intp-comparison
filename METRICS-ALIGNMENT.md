...
| **nets** | `(avg_lat_ns × count) / 1e9 × 100`, summed TX+RX. Probes: `__dev_queue_xmit`+`net:net_dev_xmit` (TX) and `__napi_schedule_irqoff`+`napi_complete_done` (RX) | ≡ V0 | ≡ V0 | ≡ V0 | **softirq fraction × softirq pct × num_cores** [softirq model, matches V0 wall-clock semantics in aggregate] | `lat_total_ns / interval_ns × 100` ≡ V0 (per-event service time, no `/num_cores`) | **`(tx_softirq_lat_ns + rx_softirq_lat_ns) / interval_ns × 100`** via `irq:softirq_entry/exit` filtered by `vec==2,3`. Diverges from V0's per-packet kprobe model because (a) veth driver xmit is microseconds — V0's `__dev_queue_xmit` kprobe captured queue+stack time which modern kernels split across separate functions; (b) backlog NAPI under sustained traffic almost never calls `napi_complete_done` so per-poll RX timing degenerates. Softirq tracepoints capture the actual CPU time in the network bottom half, equivalent to V2's procfs softirq backend. |
