/*
 * intp-helper -- userspace daemon for IntP v0.2 hardware metrics.
 *
 * v0.2 keeps the paper-faithful V0 stap script for netp/nets/blk/llcmr/cpu
 * but moves the two RCU-unsafe operations -- uncore IMC perf events and
 * cqm_rmid-based LLC occupancy -- into this userspace helper. Target
 * kernel is 5.15 GA (Ubuntu 22.04). The Canonical 5.15 kernel ships with
 * RCU-checking backports that destabilise V0's perf_event_create_kernel_counter
 * path inside stap probe context; the helper sidesteps that by opening
 * the IMC events via perf_event_open(2) and exposing LLC occupancy via
 * the resctrl mon_groups filesystem.
 *
 * The architecture mirrors v1.1-stap-helper/intp-helper.c. The two
 * substantive differences:
 *
 *   - Uncore IMC PMU types are configurable via env (the 2022 paper dev
 *     box used a single type 14; modern Sapphire Rapids under Ubuntu 22
 *     exposes multiple uncore_imc_* types). Defaults are conservative
 *     (single type) and intended to be overridden from intp-detect.sh
 *     output at launch time.
 *
 *   - The data file lives at /tmp/intp-v0.2-hw-data to avoid colliding
 *     with v1.1's /tmp/intp-hw-data if both ever coexist on the same host.
 *
 * Build: make
 ...
 *   INTP_HELPER_DRAM_BW_MBPS       nominal DRAM bandwidth in MB/s (default 34000;
 *                                  paper's 2022 dev box -- override for SPR/etc)
 *   INTP_HELPER_L3_SIZE_KB         L3 cache size in KB (default 35840 = 35 MB)
 *   INTP_HELPER_IMC_PMU_TYPE       single uncore_imc PMU type (default 14;
 *                                  paper's 2022 kernel)
 *   INTP_HELPER_IMC_PMU_TYPE_FIRST first PMU type in the IMC range, overrides
 *                                  INTP_HELPER_IMC_PMU_TYPE if set
 *   INTP_HELPER_IMC_PMU_TYPE_LAST  last PMU type in the IMC range; defaults
 *                                  to FIRST (single-type semantics)
 *   INTP_HELPER_INTERVAL_S         polling interval in seconds (default 1)
 *   INTP_HELPER_DATA_FILE          output path (default /tmp/intp-v0.2-hw-data)
 */
...
/* ---- defaults (kernel 5.15 GA / paper baseline) ----------------------- */

#define DEFAULT_DRAM_BW_MBPS  34000u    /* paper dev box; override per host */
#define DEFAULT_L3_SIZE_KB    35840u    /* 35 MB; override per host */
#define DEFAULT_IMC_TYPE         14u    /* paper kernel uncore PMU type */
#define DEFAULT_INTERVAL_S        1
#define DEFAULT_DATA_FILE     "/tmp/intp-v0.2-hw-data"

#define IMC_CONFIG_RD       0x0304u
#define IMC_CONFIG_WR       0x0c04u
#define IMC_CACHE_LINE_BYTES    64u
#define IMC_MAX_TYPES           64u
#define IMC_MAX_EVENTS         (IMC_MAX_TYPES * 2u)
...
static int imc_open_one(int type, uint64_t config) {
	...
	pe.disabled = 0;
	pe.inherit = 0;
	return (int)perf_event_open(&pe, -1, 0, -1, 0);
}

static int imc_open_all(struct imc_events *ev) {
	ev->count = 0;
	for (unsigned t = ev->type_first; t <= ev->type_last && t < ev->type_first + IMC_MAX_TYPES; t++) {
		int fd_rd = imc_open_one((int)t, IMC_CONFIG_RD);
		int fd_wr = imc_open_one((int)t, IMC_CONFIG_WR);
		if (fd_rd >= 0) ev->fds[ev->count++] = fd_rd;
		if (fd_wr >= 0) ev->fds[ev->count++] = fd_wr;
	}
	if (ev->count == 0) {
		log_warn("no IMC events opened (uncore types %u..%u unavailable); "
		         "mbw will report 0", ev->type_first, ev->type_last);
		return -1;
	}
	log_info("opened %d uncore IMC events (types %u..%u)",
	         ev->count, ev->type_first, ev->type_last);
	return 0;
}
...
static int resctrl_setup(struct resctrl_state *st) {
	...
	snprintf(st->mon_group, sizeof(st->mon_group),
	         "%s/mon_groups/intp-v02-%d", RESCTRL_ROOT, (int)getpid());
...
static void usage(const char *argv0) {
	fprintf(stderr,
	        ...
	        "  INTP_HELPER_IMC_PMU_TYPE        default %u (single-type shortcut)\n"
	        "  INTP_HELPER_IMC_PMU_TYPE_FIRST  default = IMC_PMU_TYPE\n"
	        "  INTP_HELPER_IMC_PMU_TYPE_LAST   default = FIRST (single)\n"
	        "  INTP_HELPER_INTERVAL_S          default %u\n"
	        "  INTP_HELPER_DATA_FILE           default %s\n",
	        argv0, DEFAULT_INTERVAL_S, DEFAULT_DATA_FILE,
	        DEFAULT_DRAM_BW_MBPS, DEFAULT_L3_SIZE_KB, DEFAULT_IMC_TYPE,
...
int main(int argc, char **argv) {
	...
	unsigned long imc_single = getenv_ul("INTP_HELPER_IMC_PMU_TYPE", DEFAULT_IMC_TYPE);
	unsigned long imc_first  = getenv_ul("INTP_HELPER_IMC_PMU_TYPE_FIRST", imc_single);
	unsigned long imc_last   = getenv_ul("INTP_HELPER_IMC_PMU_TYPE_LAST",  imc_first);
	if (imc_last < imc_first) {
		log_warn("IMC_PMU_TYPE_LAST < FIRST; clamping LAST to FIRST");
		imc_last = imc_first;
	}
	...
	struct config cfg = {
		...
		.imc_first    = (unsigned)imc_first,
		.imc_last     = (unsigned)imc_last,
	};
	if (!cfg.data_file || !*cfg.data_file) cfg.data_file = DEFAULT_DATA_FILE;
	if (cfg.interval_s == 0) cfg.interval_s = 1;

	log_info("pattern=\"%s\" data_file=%s dram_bw=%lu MB/s l3=%lu KB "
	         "imc=%u..%u interval=%us",
	         cfg.pattern, cfg.data_file, cfg.dram_bw_mbps,
	         cfg.l3_size_kb, cfg.imc_first, cfg.imc_last, cfg.interval_s);
	...
	struct imc_events imc = { .type_first = cfg.imc_first, .type_last = cfg.imc_last };
	imc_open_all(&imc);
